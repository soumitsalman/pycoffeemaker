...
def migrate(from_db: str, to_db: str, batch_size: int, window: int, *requested_items):
    print(
        f"[{now().strftime('%A, %Y-%m-%d')}] Migrating from {from_db} to {to_db} with batch size {batch_size} and window {window} for items: {requested_items or 'all'}"
    )

    from_db_instance = db_instance(from_db)
    to_db_instance = db_instance(to_db)

    BEAN_CONDITIONS = ["gist IS NOT NULL", "embedding IS NOT NULL"]
    ...
    items_map = {
        "beans": {
            ...
            "port_fn": lambda offset: to_db_instance.store_beans(
                from_db_instance.query_latest_beans(
                    conditions=BEAN_CONDITIONS, offset=offset, limit=batch_size
                )
            ),
        ...
        "contents": {
            ...
            "port_fn": lambda offset: to_db_instance.update_beans(
                from_db_instance.query_latest_beans(
                    ...
                    offset=offset,
                    limit=batch_size,
                    columns=[K_URL, K_CONTENT, K_CONTENT_LENGTH, K_GIST, K_SENTIMENTS],
                ),
        ...
        "publishers": {
            ...
            "port_fn": lambda offset: to_db_instance.store_publishers(
                from_db_instance.query_publishers(
                    conditions=PUBLISHER_CONDITIONS, offset=offset, limit=batch_size
                )
            ),
        ...
        "chatters": {
            ...
            "port_fn": lambda offset: to_db_instance.store_chatters(
                from_db_instance.query_chatters(
                    conditions=CHATTER_CONDITIONS, offset=offset, limit=batch_size
                )
            ),
    ...
    for name, cfg in items_map.items():
        if not requested_items or name in set(requested_items):
            total = from_db_instance.count_rows(
                cfg["count_table"], conditions=cfg["conditions"]
            )
            for offset in tqdm(
                range(0, total, batch_size),
                desc=cfg["tqdm_desc"],
                unit=cfg["count_table"],
            ):
                cfg["port_fn"](offset)
    ...
    if True:
        ...
        with tqdm(desc="Hydrating State Caches", unit="publishers") as pbar:
            while pubs := db.query_publishers(
                conditions=[
                    "(rss_feed IS NOT NULL) OR (favicon IS NOT NULL) OR (site_name IS NOT NULL) OR (description IS NOT NULL)"
                ],
                limit=batch_size,
                offset=offset,
            ):
        ...
        with tqdm(desc="Hydrating State Caches", unit="beans") as pbar:
            while beans := db.query_latest_beans(
                ...
                with ThreadPoolExecutor() as executor:
                    ...
                    executor.map(
                        lambda state: states.set("beans", state, bean_states),
                        ["collected", "embedded", "extracted", "digested"],
                    )
                offset += len(beans)
...
parser.add_argument(
    "--hydrate",
    action="store_true",
    help="Hydrate state caches instead of doing a full migration",
)
...
parser.add_argument(
    "--classifier_cache", type=str, help="Classifier cache path for hydration"
)
parser.add_argument(
    "--batch_size", type=int, default=2048, help="Batch size for migration"
)
...
parser.add_argument(
    ...
    type=str,
    nargs="*",
    help="Items to migrate (beans, publishers, chatters)",
)
