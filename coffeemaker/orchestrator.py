...
def initialize(db_conn_str: str, storage_conn_str: str, working_dir: str, emb_path: str, llm_path: str, cat_eps: float, clus_eps: float): 
    ...
    digestor = LocalDigestor(model_path=llm_path, context_len=16384) 

    global remotesack, localsack, category_eps, cluster_eps        
    remotesack = MongoSack(db_conn_str, embedder)
    localsack = DuckSack(db_dir=working_dir+"/.db", backup_conn_str=storage_conn_str)
...
def embed_beans(beans: list[Bean]) -> list[Bean]|None:   
    ...
    for bean in beans:
        try: bean.embedding = embedder.embed(bean.text)  
...
def augment_beans(beans: list[Bean]) -> list[Bean]|None:
    ...
    for bean in beans:             
        ...
        except: log.error("failed augmenting", extra={"source": bean.source, "num_items": 1})
...
def cluster_beans(source: str, beans: list[Bean]):
    if not beans: return
...
def trend_rank_beans(source: str, beans: list[Bean] = None):
    trends = find_trend_ranks([bean.url for bean in beans] if beans else None)
    if not trends: return

    num_items = update_trend_ranks(trends, run_update_time)
    log.info("trend ranked", extra={"source": source or run_id, "num_items": num_items})
...
async def run_collections_async():
    ...
    async with TaskGroup() as tg:
        ...
        [tg.create_task(collect_async(source, scraper.collect_rssfeed(source)), name=source) for source in feed_urls]
        ...
        [tg.create_task(collect_async(source, scraper.collect_subreddit(source)), name=source) for source in subreddits]
        await asyncio.sleep(TIMEOUT) # wait out to give the network a chance to recover
        log.info("collecting", extra={"source": HACKERNEWS, "num_items": 0})
        tg.create_task(collect_async(HACKERNEWS, scraper.collect_ychackernews()), name=HACKERNEWS)

async def collect_async(source: str, collect: Awaitable[list[tuple[Bean, Chatter]]]):
    ...
    beans = new_beans([bean for bean in beans if bean]) if beans else None
    needs_download = downloadables(beans)
    if needs_download:
        await download_queue.put((source, needs_download))
        beans = [bean for bean in beans if bean not in needs_download]
    
    if not beans: return
    
    await index_queue.put((source, beans))
    log.info("collected", extra={"source": source, "num_items": len(beans)})

@log_runtime
async def run_downloading_async():
    async with TaskGroup() as tg:
        while True:
            ...
            source, beans = items
            tg.create_task(download_async(source, beans), name="downloading: "+source)

async def download_async(source: str, beans: list[Bean]):
    if not beans: return
    
    downloaded_beans = indexables(await scraper.collect_beans(beans, collect_metadata=True))
    if downloaded_beans:
        await index_queue.put((source, downloaded_beans))
...
@log_runtime
async def run_indexing_async():
    total_new_beans = 0
    ranking_tasks = []
    while True:
        items = await index_queue.get()
        if items == END_OF_STREAM: break
        if not items: continue
        ...
        ranking_tasks.append(asyncio.to_thread(cluster_beans, source, beans))
        ranking_tasks.append(asyncio.to_thread(trend_rank_beans, source, beans))
...
@log_runtime
async def run_async():    
    global scraper, download_queue, index_queue, run_update_time, run_id
    scraper = AsyncCollector()
    download_queue, index_queue = Queue(), Queue()
    run_update_time = now()
    run_id = run_update_time.strftime("%Y-%m-%d %H")

    await scraper.start()

    collections_task = asyncio.create_task(run_collections_async())
    downloading_task = asyncio.create_task(run_downloading_async())
    indexing_task = asyncio.create_task(run_indexing_async())
    
    await collections_task
    await download_queue.put(END_OF_STREAM)
    # i don't need to put these on thread 
    # because by this time downloading and indexing are already running
    cleanup()
    trend_rank_beans(None, None)

    await downloading_task
    await index_queue.put(END_OF_STREAM)
    await indexing_task
