...
class Porter:
    ...
    def hydrate_beansacks(self, db: Beansack):
        ...
        if beans := self.cache.get(
            ...
            count = sum(ThreadPoolExecutor(max_workers=MAX_WORKERS).map(db.store_beans, batched([Bean(**b) for b in beans], MAX_ITEMS)))
        ...
        if related_beans := self.cache.get(
            ...
            count = sum(ThreadPoolExecutor(max_workers=MAX_WORKERS).map(db.store_related, batched(unpack_related(related_beans), MAX_ITEMS)))
