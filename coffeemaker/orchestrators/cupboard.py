...
class Cupboard:
    ...
    # def _link_events_to_sources(self, events: list[dict[str, Any]]):
    #     event_ids = self.get_node_ids_by_unique_property(EVENTS, K_URL, [item[K_URL] for item in events])
    #     source_ids = self.get_node_ids_by_unique_property(SOURCES, K_BASE_URL, list({item[K_BASE_URL] for item in items if item.get(K_BASE_URL)}))
    #     edges = [
    #         (source_ids.get(item[K_BASE_URL]), event_ids.get(item[K_URL]))
    #         for item in events
    #         if source_ids.get(item[K_BASE_URL]) and event_ids.get(item[K_URL])
    #     ]
    #     return self.batch_create_edges(edges, "PUBLISHED")

    # def _link_sources_to_events(self, sources: list[dict[str, Any]]):
    #     event_ids = self.get_node_ids_by_property(EVENTS, K_BASE_URL, [item[K_BASE_URL] for item in sources])
    #     source_ids = self.get_node_ids_by_unique_property(SOURCES, K_BASE_URL, list({item[K_BASE_URL] for item in items if item.get(K_BASE_URL)}))
        
    #     ic(list(event_ids.items())[:3], len(event_ids))
    #     edges = []
    #     for item in sources:
    #         s_id = source_ids.get(item[K_BASE_URL])
    #         e_ids = event_ids.get(item[K_URL])
    #         if s_id and e_ids:
    #             edges.extend((s_id, e) for e in e_ids)
    #     return self.batch_create_edges(edges, "PUBLISHED")
    ...
    def batch_create_edges(self, pairs: list[tuple[int, int]], edge_type: str):
        ...
        # import random
        # ic(random.sample(edges, 3), len(edges))
