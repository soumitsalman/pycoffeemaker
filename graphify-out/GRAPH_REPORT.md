# Graph Report - pycoffeemaker  (2026-07-02)

## Corpus Check
- 65 files · ~64,503 words
- Verdict: corpus is large enough that graph structure adds value.

## Summary
- 2532 nodes · 4159 edges · 186 communities (131 shown, 55 thin omitted)
- Extraction: 93% EXTRACTED · 7% INFERRED · 0% AMBIGUOUS · INFERRED: 275 edges (avg confidence: 0.78)
- Token cost: 0 input · 0 output

## Graph Freshness
- Built from commit: `eed9c685`
...
8. `_build_rss_item()` - 27 edges
9. `Consolidator` - 25 edges
...
Cohesion: 0.08
Nodes (20): _beans_to_df(), create_db(), cursor(), DuckSack, _execute_df(), _primary_key_fields(), _publishers_to_df(), Initialize a DuckSack connection.          Args:             db_path: Path to a (+12 more)
...
Cohesion: 0.08
Nodes (16): ContentGate, exclude_content(), is_empty_html(), is_excluded_content(), is_pdf(), is_pdf_content(), is_pdf_url(), parse_int() (+8 more)
...
Cohesion: 0.14
Nodes (13): APICollector, APICollectorAsync, _batch_run(), _build_reddit_json_item(), _collect_reddit_rss_entries(), _collect_rss_entries(), _fetch_json(), _fetch_rss() (+5 more)

### Community 17 - "Community 17"
Cohesion: 0.11
Nodes (30): _build_hackernews_item(), _build_reddit_item(), _build_reddit_rss_item(), cleanup_item(), extract_base_url(), guess_article_type(), guess_content_type(), remove_query_params() (+22 more)
...
Cohesion: 0.11
Nodes (20): AsyncStateCacheBase, encode_data(), _add_ts_and_ids_expr(), _copy_insert_state_rows(), _create_multi_state_query_expr(), create_query_expr(), _create_rows(), _create_single_state_query_expr() (+12 more)

### Community 21 - "Community 21"
Cohesion: 0.09
Nodes (10): create_embedder(), LlamaCppEmbeddings, ORTEmbeddings, OVEmbeddings, # NOTE: moving the import inside the function so that there is no need to instal, # NOTE: moving the import inside the function so that there is no need to instal, # NOTE: moving the import inside the function so that there is no need to instal, RemoteEmbeddings (+2 more)

### Community 22 - "Community 22"
Cohesion: 0.17
Nodes (25): _deduplicate(), generate_fake_beans(), generate_fake_chatters(), generate_fake_digests(), generate_fake_embeddings(), generate_fake_extractions(), generate_fake_mugs(), generate_fake_publishers() (+17 more)
...
Cohesion: 0.09
Nodes (23): _parse_reddit_rss_entry(), Parse Reddit RSS entry content HTML. Returns (external_url, comments_url, selfte, Parse Reddit RSS entry content HTML. Returns (external_url, comments_url, selfte, Parse Reddit RSS entry content HTML. Returns (external_url, comments_url, selfte, parse_date(), sanitize_html_for_xml(), strip_html_tags(), _get_metadata() (+15 more)
...
Cohesion: 0.1
Nodes (24): _extract_jsonld_content(), _is_parseable_html(), _is_pdf_content(), _is_pdf_url(), _parse_jsonld_body(), Scrape a single URL for both bean and publisher data., Scrape a single URL for both bean and publisher data., # TODO: add a check to remove "advertisement" (+16 more)
...
Cohesion: 0.08
Nodes (23): extract_domain(), Scrape a single site for publisher data., Scrape a single site for publisher data., Scrape a single site for publisher data., Scrape a site for publisher data., Scrape a site for publisher data., Scrape a single site for publisher data., Scrape a single site for publisher data. (+15 more)
...
Cohesion: 0.12
Nodes (19): cleanup_bean_tags(), hydrate_processing_cache(), Hydrates local processing cache with beans and publishers from production/backup, Hydrates local processing cache with beans and publishers from production/backup, Hydrates local processing cache with beans and publishers from production/backup, recitify_embeddings_and_classifications(), create_client(), db() (+11 more)
...
Cohesion: 0.17
Nodes (17): _build_rss_item(), _extract_author_email(), _extract_body(), _extract_default_rss_entries(), _extract_feed_metadata(), _extract_language(), _extract_link(), _extract_main_image() (+9 more)

### Community 35 - "Community 35"
Cohesion: 0.12
Nodes (16): AINewsDigest, _apply_model_json_constraints(), cleanup_digest_fields(), CyberNewsDigest, Digest, EarningsReportSummary, LogisticsDigest, model_json_schema() (+8 more)
...
Cohesion: 0.15
Nodes (10): Collector, Store storable collection results and persist the rest for scraping., Store storable collection results and persist the rest for scraping., Store storable collection results and persist the rest for scraping., Store storable collection results and persist the rest for scraping., Store storable collection results and persist the rest for scraping., Store storable collection results and persist the rest for scraping., Store storable collection results and persist the rest for scraping. (+2 more)
...
Cohesion: 0.13
Nodes (6): cleanup_markdown(), parse_compressed(), parse_markdown(), _strip_fences(), remove_after(), remove_before()
...
Cohesion: 0.15
Nodes (9): InfinityEmbeddings, TransformerEmbeddings, VLLMEmbedder, clear_gpu_cache(), Clear GPU memory by running garbage collection and clearing CUDA cache if availa, clear_gpu_cache(), Clear GPU memory by running garbage collection and clearing CUDA cache if availa, clear_gpu_cache() (+1 more)

### Community 48 - "Community 48"
Cohesion: 0.13
Nodes (6): DiffuserImageGenerationAgent, image_agent_from_path(), LMAgentBase, RemoteImageGenerationAgent, text2text_agent_from_path(), Text2TextAgent
...
Cohesion: 0.13
Nodes (15): create_composer_topics_locally(), migrate_classification_cache(), migrate_classification_cache_pg_to_fire(), migrate_users(), Migrate ClassificationCache from PostgreSQL to Firebird/zvec.      Args:, Migrate ClassificationCache from PostgreSQL to Firebird/zvec.      Args:, Migrate ClassificationCache from PostgreSQL to Firebird/zvec.      Args:, Read a Parquet file, split it into chunks of `chunk_size` rows and     write eac (+7 more)
...
Cohesion: 0.12
Nodes (9): Briefing, cleanup_fields(), Intelligence briefing from a stream of events., Intelligence briefing from a stream of events., Intelligence briefing from a stream of events., Intelligence briefing from a stream of events., Intelligence briefing from a stream of events., text_value() (+1 more)
...
Cohesion: 0.19
Nodes (7): LlamaCppTextGeneratorClient, LMClientBase, RemoteTextGeneratorClient, text_generator_agent(), text_generator_client(), TextGeneratorAgent, TransformerTextGeneratorClient
...
Cohesion: 0.15
Nodes (7): merge_lists(), generate_uuid(), Project-wide utilities (logging, dates, ids, fields)., log_runtime(), log_runtime_async(), structlog logfmt logging: file when LOG_DIR set, else stderr., _runtime_decorator()

### Community 62 - "Community 62"
Cohesion: 0.13
Nodes (14): Augment existing publishers with scraped data., Augment existing publishers with scraped data., Augment existing publishers with scraped data., Prepare result for page scraping (bean and publisher)., Augment existing publishers with scraped data., Augment existing publishers with scraped data., Scrape multiple URLs in parallel for bean and publisher data., Augment existing publishers with scraped data. (+6 more)

### Community 63 - "Community 63"
Cohesion: 0.13
Nodes (14): Collects the bodies of the urls as markdowns, Collects the bodies of the urls as markdowns, Collects the bodies of the urls as markdowns, Collects the body of the url as a markdown, Collects the bodies of the urls as markdowns, Collects the bodies of the urls as markdowns, Collects the bodies of the urls as markdowns, Collects the bodies of the urls as markdowns (+6 more)

### Community 64 - "Community 64"
Cohesion: 0.13
Nodes (14): Collects the bodies of the beans as markdowns, Collects the bodies of the beans as markdowns, # TODO: add a check to remove "advertisement", Collects the bodies of the beans as markdowns, Collects the bodies of the beans as markdowns, Collects the bodies of the beans as markdowns, Collects the bodies of the beans as markdowns, Collects the bodies of the urls as markdowns (+6 more)

### Community 65 - "Community 65"
Cohesion: 0.13
Nodes (14): # TODO: add a check to remove "advertisement", Collects the body of the url as a markdown, Collects the body of the url as a markdown, Collects the body of the url as a markdown, Collects the body of the url as a markdown, Collects the body of the url as a markdown, Collects the body of the url as a markdown, Collects the body of the url as a markdown (+6 more)

### Community 66 - "Community 66"
Cohesion: 0.13
Nodes (14): cleanup_names(), Remove leading/trailing non-alphanumeric characters, filter out empty and undete, Remove leading/trailing non-alphanumeric characters, filter out empty and undete, Converts the tags into snake_case, Converts the tags into snake_case, Remove leading/trailing non-alphanumeric characters, filter out empty and undete, Converts the tags into snake_case, Remove leading/trailing non-alphanumeric characters, filter out empty and undete (+6 more)

### Community 67 - "Community 67"
Cohesion: 0.16
Nodes (5): _prepare_to_store(), Opens connection to existing or new simple database on `db_path`.         Create, SimpleVectorDB, test_static_db(), test_static_db()

### Community 68 - "Community 68"
Cohesion: 0.2
Nodes (12): create_micro_agent(), create_text_analyst(), _load_json(), NLP embedder / digestor / extractor smoke tests (GPU/network)., _save_json(), test_digestor(), test_embedder(), test_extractor() (+4 more)

### Community 70 - "Community 70"
Cohesion: 0.18
Nodes (7): Config, # NOTE: this is deprecated. IGNORE, # NOTE: something wrong with the vector index creation, Generated article stored in cupboard, _Sip, Sip, load_espresso_to_cupboard()
...
Cohesion: 0.18
Nodes (10): _Bean, _fix_publisher_ids(), _Publisher, # TODO: split out function for adding embeddings and gists. code commented out b, # TODO: add function for recompute (for clusters, categories, sentiments and tre, # TODO: add a recompute and cleanup function, # TODO: add delete for bookmarked bean, # NOTE: remove anything collected 7 days ago that did not get processed by analy (+2 more)
...
Cohesion: 0.14
Nodes (13): Prepare result for page scraping (bean and publisher)., Prepare result for page scraping (bean and publisher)., Prepare result for page scraping (bean and publisher)., Prepare result for page scraping (bean and publisher)., Prepare result for page scraping (bean and publisher)., Prepare result for page scraping (bean and publisher)., Prepare result for page scraping (bean and publisher)., Prepare result for page scraping (bean and publisher). (+5 more)

### Community 76 - "Community 76"
Cohesion: 0.14
Nodes (13): Scrape a single site for publisher data., Scrape a single publisher for publisher data., Scrape a single publisher for publisher data., Scrape a single publisher for publisher data., Scrape a single publisher for publisher data., Scrape a single publisher for publisher data., Scrape a single publisher for publisher data., Scrape multiple URLs in parallel for bean and publisher data. (+5 more)
...
Cohesion: 0.21
Nodes (8): is_cuda_oom(), is_cuda_oom(), is_cuda_oom(), clean_updates(), Clusterer, decache_beans(), _is_cuda_oom(), run()

### Community 82 - "Community 82"
Cohesion: 0.15
Nodes (12): Scrape a single bean for page data., Scrape a single bean for page data., Scrape a single bean for page data., Scrape a single bean for page data., Scrape a single bean for page data., Scrape a single bean for page data., Scrape a single bean for page data., Scrape a single publisher for publisher data. (+4 more)
...
Cohesion: 0.15
Nodes (12): Scrape multiple URLs in parallel for bean and publisher data., Scrape multiple URLs in parallel for bean and publisher data., Scrape multiple URLs in parallel for bean and publisher data., Scrape multiple URLs in parallel for bean and publisher data., Scrape multiple URLs in parallel for bean and publisher data., Scrape multiple URLs in parallel for bean and publisher data., Scrape multiple URLs in parallel for bean and publisher data., Scrape multiple URLs in parallel for bean and publisher data. (+4 more)
...
Cohesion: 0.24
Nodes (6): BeansackPorter, prep_beans(), Ports beans, publishers and related beans to 1 or more Beansacks, Ports beans, publishers and related beans to 1 or more Beansacks, Ports beans, publishers and related beans to 1 or more Beansacks, Ports beans, publishers and related beans to 1 or more Beansacks

### Community 90 - "Community 90"
Cohesion: 0.17
Nodes (11): Async context manager enter, Async context manager enter, Async context manager enter, Async context manager enter, Async context manager enter, Async context manager enter, Async context manager enter, Async context manager enter (+3 more)

### Community 91 - "Community 91"
Cohesion: 0.27
Nodes (5): AsyncWebScraper, Scrape a PDF and convert it to markdown content., Scrape a PDF and convert it to markdown content., Sync: single lxml parse for metadata only., _title_from_url()
...
Cohesion: 0.22
Nodes (4): Cupboard, Link a list of sips to a list of other sips.          Args:             - `relat, Link a list of sips to a list of other sips.          Args:             - `relat, Link a list of sips to a list of other sips.          Args:             - `relat
...
Cohesion: 0.25
Nodes (4): prep_events(), prep_related(), prep_signals(), merge_tags()
...
Cohesion: 0.29
Nodes (7): _convert_pdf_to_markdown(), Process worker: convert a PDF file to markdown., Process worker: convert a PDF file to markdown., Process worker: convert a PDF file to markdown., Process worker: convert a PDF file to markdown., Process worker: convert a PDF file to markdown., Process worker: convert a PDF file to markdown.
...
Cohesion: 0.33
Nodes (5): _clean_null_bytes(), Store a list of sips in the database., Store a list of sips in the database., Store a list of sips in the database., Recursively remove null bytes (\\u0000) from all strings in the object.
...
## Knowledge Gaps
- **721 isolated node(s):** `Seed cache with classification embeddings`, `Read a Parquet file, split it into chunks of `chunk_size` rows and     write eac`, `Migrate ClassificationCache from PostgreSQL to Firebird/zvec.      Args:`, `Hydrates local processing cache with beans and publishers from production/backup`, `Consolidates events and data to create consolidated briefings and signals` (+716 more)
...
- **What is the exact relationship between `Publisher` and `PUBLISHED graph edge`?**
  _Edge tagged AMBIGUOUS (relation: semantically_similar_to) - confidence is low._
- **Why does `now()` connect `Community 17` to `Community 32`, `Community 1`, `Community 2`, `Community 34`, `Community 64`, `Community 75`, `Community 77`, `Community 13`, `Community 84`, `Community 29`, `Community 95`?**
